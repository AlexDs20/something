...
static inline f32x2 f32x2_normalize(f32x2 a)                    { ASSERT(f32x2_length(a)!=0.0f); f32 len = f32x2_length(a); if (len > F32_EPSILON) { f32 inv = 1.0f / len; return {a.x*inv, a.y*inv}; } else { return {0.0f, 0.0f}; } }
...
static inline f32x3 operator/(f32x3 a, f32x3 b)                 { return {a.x / b.x, a.y / b.y, a.z / b.z}; }
static inline f32x3 operator/(f32   a, f32x3 b)                 { return {a   / b.x, a   / b.y, a   / b.z}; }
static inline f32x3 operator/(f32x3 a, f32   b)                 { f32 inv = 1.0f / b; return {a.x * inv, a.y * inv, a.z * inv}; }
...
static inline f32x3 f32x3_normalize(f32x3 a)                    { f32 len = f32x3_length(a); if (len > F32_EPSILON) { f32 inv = 1.0f / len; return {a.x * inv, a.y * inv, a.z * inv}; } else { return {0.0f, 0.0f, 0.0f}; } }
