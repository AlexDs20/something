...
static inline f32x2 f32x2_normalize(f32x2 a)                    { f32 len2 = f32x2_length2(a); ASSERT(len!=0.0f); if (len2 > F32_EPSILON) { f32 inv = f32_rsqrt(len2); return {a.x*inv, a.y*inv}; } else { return {0.0f, 0.0f}; } }
